						   ...
						   double lambdaA, double lambdaT, double lambdaW)
{
	...
	if (width > height)
		...
		aspect = height / width;
        (void)aspect; /* Avoid -Wunused-but-set-variable */
	free_dvector(temp);
...
flp_t *read_flp(char *file, int read_connects)
{
	...
	if (!fp) {
        ...
        return read_flp_stream(fp, read_connects);
}


flp_t *read_flp_stream(FILE *fp, int read_connects)
{
	char str[STR_SIZE];
	flp_t *flp;
	int count, i, j;
        ...
        if (!fp || ferror(fp)) {
            sprintf(str, "invalid FILE stream: %p\n", fp);
            fatal(str);
        }
